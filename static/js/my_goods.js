...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
		...
		tpl.find('*[item="markprice"]').append('<input type="hidden" name="markprice['+ item.spec_id +']" value="' + item.markprice + '" />' + item.markprice);
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
		...
		tpl.find('*[item="markprice"]').val(item.markprice);
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
			...
			var markprice = $.trim($(this).find('*[item="markprice"]').val());
            ...
            if(SPEC.spec_qty == 1){ // 一个规格
                ...
                eval('if(spec_' + spec_pos + ' || (!spec_' + spec_pos + ' && !price && !markprice && !stock && !sku)){}else{complate = false;}');
            }else{ // 两个规格
                if((spec_1 && spec_2) || (!spec_1 && !spec_2 && !price && !markprice && !stock && !sku)){
			...
			if(isNaN(markprice) || markprice <0 || !markprice){
                valid && price_error.push(item);
            }
            item != ';' && SPEC.specs.push({
				...
				'markprice':number_format(markprice, 2),
