...
function zsc_show(id){
	...
	if(zsc_class!='add'){
		...
		click_id = id;
		var zsc_id = id;
		var url = "./admin.php?m=keyword&a=addkeyword_ajax&kid="+zsc_id;
		$.post(url,null,function(data){
			$.each(data,function(i,v){
				...
				zsc_html = "<div class=\"edit-info\">"+
						   ...
						   "</div>"+
						   "<div style=\" padding-top:45px; clear:both;\">"+
						   	"<span id=\"zsc_titles0\">描述</span>"+
							"<div class=\"replycon\">"+
							"<textarea id=\"zsc_kecontent1\" style=\"width:695px;height:137px; border:#ccc solid 1px;\"></textarea>"+
							"</div>"+
				...
				if(v.type==1){
				    ...
				    $('#zsc_kecontent1').val(v.kecontent);
					check_tuwen(2);
...
function choseImg(){
	...
	$('.zhe').show();
	$('.imgbox a').remove();
	var url = "./admin.php?m=keyword&a=allimages";
	$.post(url,null,function(data){
		$.each(data,function(i,v){
...
function delImg(num){
	...
	var url = "./admin.php?m=keyword&a=delimg";
...
function btnsave(num,count){
	...
	if(ktype==1){
		...
		if(count==0){
			var url = "./admin.php?m=keyword&a=addkeyword&op=add";
			$.post(url,{kename:kname,keword:kword,kecontent:kecontent,ketype:ktype},function(data){
				alert('添加成功！');
				window.location.reload();
			});
		}else{
			var url = "./admin.php?m=keyword&a=addkeyword&op=update";
			var kid = $('#zsc_iid').val();
		...
		if(count==0){
			var kecontent = $('#zsc_kecontent1').val();
			var url = "./admin.php?m=keyword&a=addkeyword&op=add";
			
			$.post(url,{kename:kname,keword:kword,kecontent:kecontent,ketype:ktype,titles:arrTitles,imageinfo:arrImgurls,linkinfo:arrCurl},function(data){
				alert('添加成功！');
				window.location.reload();
			});
		}else{
			...
			var url = "./admin.php?m=keyword&a=addkeyword&op=update";
			var kid = $('#zsc_iid').val();
			$.post(url,{kename:kname,keword:kword,kecontent:kecontent,ketype:ktype,titles:arrTitles,imageinfo:arrImgurls,linkinfo:arrCurl,kid:kid},
			function(data){
				alert('修改成功！');
				window.location.reload();
			});
...
function zscdel(num){
	...
	var url = "./admin.php?m=keyword&a=addkeyword&op=del";
