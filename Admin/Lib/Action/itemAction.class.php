...
class itemAction extends backendAction {
	...
	public function add()
    {
           ...
           /*$auid = $this->_post('auid', 'intval');
			...
			$specs = array(); // 原始规格
			switch ($data['spec_qty'])
			{

            case 1: // 一个规格
                $data['spec_name_1'] = $spec_name_1 ? $spec_name_1 : $spec_name_2;
                $data['spec_name_2'] = '';
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
								...
								'markprice'  => $this->_filter_price($_POST['markprice'][$key]),
                ...
                break;
            case 2: // 二个规格
                $data['spec_name_1'] = $spec_name_1;
                $data['spec_name_2'] = $spec_name_2;
		...
		$this->_spec_mod->where("item_id='".$item_id."'")->delete();
    ...
    public function spec_list() 
	{
			...
			$list = $this->_spec_mod->where(array('item_id'=>$id))->select();
	    ...
	    public function spec_edit() 
	{
		...
		$mod = D('item_spec');
        $pk = $mod->getPk();
        $spec_id = $this->_get('id', 'trim');
        $field = $this->_get('field', 'trim');
        $val = $this->_get('val', 'trim');
        //允许异步修改的字段列表  放模型里面去 TODO
        $mod->where(array($pk=>$spec_id))->setField($field, $val);
        $this->ajaxReturn(1,'aaaaaaaaaaaaa','bacs','test');
    ...
    public function edit() {
        if (IS_POST) {
			...
			//lishuai
			$item_spec = $this->_spec_mod->where(array('item_id'=>$id))->select();
			$spec_json= json_encode(array(
            'spec_qty' => $item['spec_qty'],
            'spec_name_1' => isset($item['spec_name_1']) ? $item['spec_name_1'] : '',
            'spec_name_2' => isset($item['spec_name_2']) ? $item['spec_name_2'] : '',
            'specs' => $item_spec,
				));
	...
	public function item_logo() {
		...
		$path = "data/rq/";
        $id = $this->_get('id');
		$value='http://'.$_SERVER['HTTP_HOST'].'/index.php?token='.session('token').'&m=item&a=index&id='.$id;
		$fileName = $path.session('token').$id.'_emall.png';
		$QR_Logo=$path.session('token').$id.'_emall_Logo.png'; 
