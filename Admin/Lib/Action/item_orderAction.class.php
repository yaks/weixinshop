...
class item_orderAction extends backendAction {
      ...
      if($this->_mod->where("orderId='".$orderId."'")->save($data))
      	{
      		...
      		//$item=M('item');
			$user=M('user');
	     	$order_details = $this->_mod->where("orderId='".$orderId."'")->find();
	    	//foreach ($order_details as $val)
	     	//{
	     	//$item->where('id='.$val['itemId'])->setInc('buy_num',$val['quantity']);
	        //}
			$member_level=M('member_level');
			$level_flag=$member_level->where("token='".session('token')."'")->find();
			if($level_flag['level_flag'])
			{ 
				
			    $score = ceil($order_details['order_sumPrice']/$level_flag['rate']);
				
				$user->where('id='.$order_details['userId'])->setInc('score',intval($score));
			}
    ...
    {
      $orderId= $this->_get('id', 'trim');
      !$orderId && $this->_404();
		
	$status_item=$this->_mod->where("orderId='".$orderId."'")->find();
      
      if(intval($status_item['status'])<3)
      {
      	$data['status']=5;
		if($this->_mod->where("orderId='".$orderId."'")->save($data))
      	{
      	...
      	{
      		IS_AJAX && $this->ajaxReturn(0, L('操作失败'));
            $this->error(L('操作失败'));
      	}
      ...
      }else 
      {
			...
			IS_AJAX && $this->ajaxReturn(0, L('发货后,不能取消'));
            $this->error(L('发货后,不能取消'));
      	}
    }
        ...
        if (IS_POST&&$this->_post('orderId','trim')) {
           ...
           /* if( $addid=$mod->add($data) ){
            ...
            if(count(M('address')->where('status=1')->find())==0)
             ...
             $deliveryList=	M('delivery')->where("status=1 and token='".session('token')."'")->order('ordid asc,id asc')->select();//快递方式
              $this->assign('deliveryList',$deliveryList);
              $addressList=M('address')->where("status=1 and token='".session('token')."'")->order('ordid asc,id asc')->select();//快递方式
