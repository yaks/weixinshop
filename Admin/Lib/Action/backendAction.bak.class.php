...
class backendAction extends baseAction
{
    ...
    public function index() {
        ...
        $this->opr_log("user opr ".$this->_name." success");
        $this->display();
    }
    ...
    public function add() {
        $mod = D($this->_name);
		$this->opr_log("user add ".$this->_name." success");
        if (IS_POST) {
    ...
    public function edit()
    {
        $mod = D($this->_name);
        $pk = $mod->getPk();
		$this->opr_log("user edit ".$this->_name." success");
        if (IS_POST) {
            if (false === $data = $mod->create()) {
                IS_AJAX && $this->ajaxReturn(0, $mod->getError());
                $this->error($mod->getError());
    ...
    public function delete()
    {
        $mod = D($this->_name);
        $pk = $mod->getPk();
		$this->opr_log("user delete ".$this->_name." success");
    ...
    protected function update_config($new_config, $config_file = '') {
	    ...
	    public function opr_log($opr_c) {
		$opr_log = M("opr_log");
		$u_t_data['token']=session('token');
		$u_t_data['user']=session('admin')['username'];
		$u_t_data['opr_time']=time();
		$u_t_data['opr_ip']=get_client_ip();
		$u_t_data['opr_content']=$opr_c;
		$opr_log->add($u_t_data);
		return true;
