               ...
               <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbdetail">
                    <tbody>
                        <tr>
						 ...
						 <?php case "3": ?>支付宝转账<?php break;?>
						 <?php case "4": ?>银行转账<?php break;?>
                        <?php default: ?>-<?php endswitch;?>
						<?php if($info["supportmetho"] == 4): echo ($info["bank_info"]); endif; ?>
                           </td>
                   ...
                   <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblist">
                    ...
                    <tbody>
                     <?php if(is_array($order_detail)): $i = 0; $__LIST__ = $order_detail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="">
                            <td><img width="68" alt="nopic" src="<?php echo attach(get_thumb($vo['img'], '_b'), session('token').'/item');?>"></td>
                            <td class="ltd">
                                <?php echo ($vo["title"]); ?> <?php echo ($vo["spec_1"]); ?> <?php echo ($vo["spec_2"]); ?> 
		...
		<div class="mt10" style="text-align:center;">
		  <?php switch($info["status"]): case "1": ?><!-- 待付款-->
               ...
               <a data-height="130" data-width="650"
                 data-id="add" data-title="发货管理" data-uri="./admin.php?m=item_order&amp;a=fahuo&amp;id=<?php echo ($info["id"]); ?>" href="javascript:void(0);" class="add fb J_showdialog"> 	<input type="button" value="发货" id="dosubmit" name="dosubmit" class="btn btn_submit" ></a><?php break;?>  
                <?php case "3": ?><!-- 待收货 -->
