...
<div class="pad_lr_10" >
    ...
    <?php if($sm == 'image'): ?><div class="J_tablelist item_imglist clearfix">
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="item fl">
            ...
            <div class="img clearfix"><img src="<?php echo attach(get_thumb($val['img'], '_m'), ''.session('token').'/item');?>"></div>
            </label>
            ...
            <ul>
                ...
                <li><a class="J_tooltip btn_blue" title="<?php echo ($val["goods_stock"]); ?>">库存</a></li>
            </ul>
    ...
    <table width="100%" cellspacing="0">
    	...
    	<tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td align="center" style ="display:none;"><?php echo ($val["id"]); ?></td>
                <td align="right">
                    <?php if(!empty($val['img'])): ?><div class="img_border"><img src="<?php echo attach(get_thumb($val['img'], '_s'), ''.session('token').'/item');?>" width="32" width="32" class="J_preview" data-bimg="<?php echo attach($val['img'], ''.session('token').'/item');?>"></div><?php endif; ?>
                </td>
             ...
             <?php if($val["spec_qty"] == 0): ?><td align="center"><b><?php echo ($val["buy_num"]); ?></b></td>
			 <td align="center"><b><?php echo ($val["goods_stock"]); ?></b></td>
             <td align="center" class="red"><?php echo ($val["price"]); ?></td><?php endif; ?>
             <?php if($val["spec_qty"] != 0): ?><td align="center"><a href="javascript:;" class="J_showdialog" data-uri="<?php echo u('item/spec_list', array('id'=>$val['id']));?>" data-title="商品明细" data-id="edit" data-width="520" data-height="330">查看库存</a></td>
			 <td align="center"><a href="javascript:;" class="J_showdialog" data-uri="<?php echo u('item/spec_list', array('id'=>$val['id']));?>" data-title="商品明细" data-id="edit" data-width="520" data-height="330">查看售量</a></td>
             <td align="center"><a href="javascript:;" class="J_showdialog" data-uri="<?php echo u('item/spec_list', array('id'=>$val['id']));?>" data-title="商品明细" data-id="edit" data-width="520" data-height="330">查看价格</a></td><?php endif; ?>			 
				...
				<td align="center"><img data-tdtype="toggle" data-id="<?php echo ($val["id"]); ?>" data-field="dis_flag" data-value="<?php echo ($val["dis_flag"]); ?>" src="__STATIC__/images/admin/toggle_<?php if($val["dis_flag"] == 0): ?>disabled<?php else: ?>enabled<?php endif; ?>.gif" /></td>
                <td align="center"><?php echo (date('Y-m-d H:i:s',$val["add_time"])); ?></td>
                <td align="center"><a href="javascript:;" class="J_showdialog" data-uri="<?php echo u('item/item_logo', array('id'=>$val['id']));?>" data-title="商品预览" data-id="edit" data-width="250" data-height="250">预览</a> | <a href="<?php echo u('item/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>"><?php echo L('edit');?></a> | <a href="javascript:void(0);" class="J_confirmurl" data-uri="<?php echo u('item/delete', array('id'=>$val['id']));?>" data-acttype="ajax" data-msg="<?php echo sprintf(L('confirm_delete_one'),$val['title']);?>"><?php echo L('delete');?></a></td>
