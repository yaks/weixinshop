...
class userAction extends userbaseAction {
    ...
    public function login() {
        ...
        if (IS_POST) {
            ...
            if (IS_AJAX) {
				...
				$user_1 = M('diymen_set','imicms_','mysql://bismai:weixinshop@zhaomingyuinter.mysql.rds.aliyuncs.com/bismai');	
				$user_data = $user_1->where("token='".$token."'")->find();
    ...
    */
    public function userorder() {
      ...
      {
      	$status=1;
		$item_orders= $item_order->order('id desc')->where("userId='".$this->visitor->info['id']."' and token = '".session('user_token')."'")->select();
      }
      else 
      {
      	$status=$_GET['status'];
		$item_orders= $item_order->order('id desc')->where("status='".$status."' and userId='".$this->visitor->info['id']."' and token = '".session('user_token')."'")->select();
      }
      ...
      foreach ($item_orders as  $key=>$val)
      {
...

      	{
      	 ...
      	 $items= array('comment_flag'=>$comment_flag,'title'=>$val['title'],'img'=>$val['img'],'price'=>$val['price'],'quantity'=>$val['quantity'],'itemId'=>$val['itemId'],'spec_1'=>$val['spec_1'],'spec_2'=>$val['spec_2']);
	...
	public function index() {
      ...
      $item_order=M('item_order');
	  $user=M('user');
	  $status1= $item_order->where("status= 1 and userId='".$this->visitor->info['id']."' and token = '".session('user_token')."'")->count('1');
	  $status4= $item_order->where("status= 2 and userId='".$this->visitor->info['id']."' and token = '".session('user_token')."'")->count('1');
	  $status2= $item_order->where("status= 3 and userId='".$this->visitor->info['id']."' and token = '".session('user_token')."'")->count('1');
	  $status3= $item_order->where("userId='".$this->visitor->info['id']."' and token = '".session('user_token')."'")->count('1');
	  $user_info=M('user')->where('id='.$this->visitor->info['id'])->find();
