...
class orderAction extends userbaseAction {
	...
	public function confirmOrder()//确认收货
	{
	     ...
	     if($item_order->where("orderId='".$orderId."' and userId='".$this->visitor->info['id']."'")->save($data))
	     {
	        ...
	        //}
			$user=M('user');
	     	$order_details = $item_order->where("orderId='".$orderId."'")->find();
	    
			$member_level=M('member_level');
			$level_flag=$member_level->where("token='".session('user_token')."'")->find();
			if($level_flag['level_flag'])
			{
				 $score = ceil($order_details['order_sumPrice']/$level_flag['rate']);
				$user->where('id='.$order_details['userId'])->setInc('score',$score);
			}
		...
		public  function checkOrder()//查看订单
	{
	  ...
	  $item_order=M('item_order');
	  $order=$item_order->where("orderId='".$orderId."' and userId='".$this->visitor->info['id']."'")->find();
      $item_comment=M('item_comment');
      $comment_flag = 0;
	  if(!is_array($order))
	  {
	  	$this->error($orderId.'该订单不存在');
	  }else 
	  {
	  	...
	  	foreach ($order_details as $val)
	  	{
            ...
            if ($status==4)
             {	  
                $comment=$item_comment->where('item_id='.$val['itemId'].' and uid='.$this->visitor->info['id'])->find();
                if (is_array($comment))
                {
                    $comment_flag = 1;
                }
	  		...
	  		$items= array('title'=>$val['title'],'img'=>$val['img'],'price'=>$val['price'],'quantity'=>$val['quantity'],'comment_flag'=>$comment_flag,'item_id'=>$val['itemId'],'spec_1'=>$val['spec_1'],'spec_2'=>$val['spec_2']);
		   ...
		   $data['userName']=$this->visitor->info['username'];//用户名
			...
			{	
				...
				foreach ($_SESSION['cart'] as $item )
				{
					...
					$orders['spec_id']=$item['spec_id'];
					$orders['spec_1']=$item['spec_1'];
					$orders['spec_2']=$item['spec_2'];
					if (intval($item['spec_id']))
					{
					    $item_num=$item_spec->where("spec_id='".$item['spec_id']."'")->find();
						$item_spec->where("spec_id='".$item['spec_id']."'")->setInc('buy_num',$item['num']);
						$item_spec->where("spec_id='".$item['spec_id']."'")->setDec('stock',$item['num']);
					}else{
						$item_goods->where('id='.$item['id'])->setDec('goods_stock',$item['num']);
						$item_goods->where('id='.$item['id'])->setInc('buy_num',$item['num']);
						$item_num=$item_goods->where('id='.$item['id'])->find();
					}
					$orders['sku']=$item_num['sku'];
				...
				$user_paymode=M('user_paymode');
				$alipay=M('alipay');
				$user_paymodes=$user_paymode->where("token='".session('user_token')."'")->find();
				if(is_array($user_paymodes)){
					if($user_paymodes['alipay']){
					$alipays=$alipay->where("token='".session('user_token')."'")->find();
					$this->assign('alipays',$alipays);
					}
					if($user_paymodes['bankpay']){
					$user_banks=$user_bank->where("token='".session('user_token')."'")->select();
					$this->assign('user_banks',$user_banks);
					}
					
				}else{
					$user_paymodes['otherpay']=1;
					$user_paymodes['alipay']=0;
					$user_paymodes['bankpay']=0;
					}
				$this->assign('user_paymodes',$user_paymodes);
	...
	public  function end()
	{
		if(IS_POST)
		{
			...
			if($payment_id==3)//货到付款
			{
				$data['status']=2;
				$data['supportmetho']=3;
				$data['ali_name']=$alipayname;
