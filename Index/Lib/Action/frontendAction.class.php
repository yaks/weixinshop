...
class frontendAction extends baseAction {
    ...
    private  function _index_cate()
    {
        ...
        //if (false === $index_cate_list = F('index_cate_list')) {
            ...
            //推荐到首页的大类
            $index_cate_list = $item_cate_mod->field('id,name,img')->where(array('pid'=>'0' ,'is_index'=>'1', 'status'=>'1' ,'token'=>session('user_token')))->order('ordid')->select();
    ...
    */
    ...
    public function wall_ajax($where = array(), $order = 'id DESC', $field = '') {
        ...
        $members=$model->table('weixin_user a,weixin_member_setting b')
              ...
              ->field('LEFT(b.discount/10 ,3) as discount')
              ->find();
			  
		if ($members)
		{ 
		   $this->assign('discount', $members['discount']);
		   
		}else{
		
			$this->assign('discount', 10);
		}
