...
*/
      ...
      /*  $item_comment_mod = M('item_comment');
		...
		$item_comment_mod = M('item_comment');
		$map = array('item_id' => $id);
        $count_comment = $item_comment_mod->where($map)->count('id');
		
		$comment_detail=M('item_comment');
	    $comment_list = $comment_detail->where('item_id='.$id)->select();
	 ...
	 public function index() {
        ...
        !$id && $this->_404();
        $item_mod = M('item');
		$item_spec_mod = M('item_spec');
		$item_img_mod = M('item_img');
        $item = $item_mod->where(array('id' => $id, 'status' => 1))->find();
        !$item && $this->_404();
		
		
		
		$item_img=$item_img_mod->where(array('item_id' => $id))->select();
		$data['item']=$item;
		$data['item_img']=$item_img;
		$data['spec_qty']=$item['spec_qty'];
		if($item['spec_qty'] !='0'){
			$item_spec = $item_spec_mod->where(array('item_id' => $id))->select();
			$data['item_spec']=$item_spec;
			}
		$json_item = json_encode($data);
		$this->assign('user_token', session('user_token'));
		$this->assign('json_item', $json_item);
		$this->memberlevel();
		$this->display();
	...
	if (IS_POST) {
		...
		$data['score'] = $this->_post('score', 'intval');
        ...
        if ($comment_id) {
			...
			$this->ajaxReturn(1, L('comment_OK'));
